...
def pit_loss_multispk(logits: List[torch.Tensor],
    ...
    Args:
        ...
        n_speakers (np.ndarray): List with the number of speakers in each batch.
    ...
    if logits.shape != target.shape:
        ipdb.set_trace()
        #raise ValueError("Logits and target shapes must be the same")
    target = target.float()
    ...
    # shape of cost_matrices is [batch_size, n_speakers, n_speakers]
    ...
    for i, cost_matrice in enumerate(cost_matrices):
        ...
        # Assert if the permutation is correct
